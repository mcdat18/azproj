...
WITH
(
    LOCATION='/contsoretaildw/DimProduct/'
,   DATA_SOURCE = DlakeDevBronze
...
WITH
(
    LOCATION='/contsoretaildw/FactOnlineSales/'
,   DATA_SOURCE = DlakeDevBronze
,   FILE_FORMAT = TextFileFormat
,   REJECT_TYPE = VALUE
,   REJECT_VALUE = 0
...
AS SELECT * FROM [retailSTG].[DimProduct] 
OPTION (LABEL = 'CTAS : Load [retail].[DimProduct]');
GO

CREATE TABLE [retail].[FactOnlineSales]
WITH (DISTRIBUTION = HASH([ProductKey]))
AS SELECT * FROM [retailSTG].[FactOnlineSales]
OPTION (LABEL = 'CTAS: Load [retail].[FactOnlineSales]')
GO